...
parser.add_argument(
    "--vtype", type=str, default="clip", help="imagenet | places | emotion | clip"
)
parser.add_argument(
    "--ttype", type=str, default="clip", help="bertbase | robertabase | clip"
)
...
parser.add_argument("--epochs", type=int, default=600, help="50, 75, 100")
...
parser.add_argument("--layer", type=str, default="sumavg", help="sumavg, 2last, last")
...
def train(model, optimizer, lr_scheduler, num_epochs):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for inputs1, inputs2, labels in tr_loader:
            ...
            # print("PA TYPE ", type(outputs), type(labels), type(smooth))
...
te_ids = (
    pd.read_csv(dloc + "splits/test_%d.txt" % (split), header=None).to_numpy().flatten()
)
...
def main():
    ...
    print(
        "Best Epoch: %d, Test Acc: %.4f, %.4f, %.4f"
        % (best_epoch, np.round(te_loss, 4), np.round(te_acc, 4), np.round(te_f1, 4))
    )
